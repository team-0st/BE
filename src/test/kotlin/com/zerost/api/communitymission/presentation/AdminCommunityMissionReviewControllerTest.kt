...
class AdminCommunityMissionReviewControllerTest {
    ...
    fun `관리자는 검수 대기 공동 미션 인증 목록을 조회할 수 있다`() {
        `when`(adminCommunityMissionReviewQueryService.getPendingProofs()).thenReturn(
            listOf(
        ...
        mockMvc.perform(get("/api/v1/admin/community-missions/proofs/pending"))
            ...
            .andExpect(jsonPath("$.data[0].proofId").value(101))
            .andExpect(jsonPath("$.data[0].communityMissionId").value(3))
            .andExpect(jsonPath("$.data[0].requirementId").value(11))

        verify(adminCommunityMissionReviewQueryService).getPendingProofs()
    }
