...
class RecipeQueryServiceTest {
    ...
    @Test
    fun `레시피 목록 조회 시 히든과 전설 레시피 이름은 마스킹된다`() {
        ...
        `when`(soupRepository.findDistinctRecipeIdsByUserId(1L)).thenReturn(emptyList())
        val introRecipe = createRecipe(id = 1L, name = "따뜻한 입문 스프", type = RecipeType.COMMON, intro = true)
        val generalRecipe = createRecipe(id = 2L, name = "오리지널 스프", type = RecipeType.COMMON)
        val hiddenRecipe = createRecipe(id = 3L, name = "크리스탈 스프", type = RecipeType.HIDDEN, hidden = true)
        val legendaryRecipe = createRecipe(id = 4L, name = "별의 축복 스프", type = RecipeType.LEGENDARY, hidden = true, slotCount = 5)
        `when`(recipeRepository.findAllByOrderByIdAsc()).thenReturn(
            listOf(introRecipe, generalRecipe, hiddenRecipe, legendaryRecipe),
        )
        ...
        assertEquals("오리지널 스프", response.generalRecipes[0].name)
        ...
        assertEquals("???", response.legendaryRecipes[0].name)
        assertEquals(false, response.legendaryRecipes[0].recipeVisible)
    }

    @Test
    fun `레시피 목록 조회 시 입문이 아닌 일반 레시피는 보통 레시피 섹션에 포함된다`() {
        `when`(userRepository.findById(1L)).thenReturn(Optional.of(createUser()))
        `when`(userUnlockedRecipeRepository.findRecipeIdsByUserId(1L)).thenReturn(emptyList())
        `when`(soupRepository.findDistinctRecipeIdsByUserId(1L)).thenReturn(emptyList())
        `when`(recipeRepository.findAllByOrderByIdAsc()).thenReturn(
            listOf(
                ...
                createRecipe(id = 5L, name = "전설의 스프", type = RecipeType.LEGENDARY, hidden = true, slotCount = 5),
            ),
        )
        ...
        assertEquals(1, response.introRecipes.size)
        assertEquals(1, response.generalRecipes.size)
        assertEquals("숨은 일반 스프", response.generalRecipes[0].name)
        assertEquals(1, response.hiddenRecipes.size)
        assertEquals(1, response.legendaryRecipes.size)
    }
    ...
    @Test
    fun `조합 성공한 전설 레시피는 목록과 상세에서 공개된다`() {
        val legendaryRecipe = createRecipe(id = 5L, name = "별의 축복 스프", type = RecipeType.LEGENDARY, hidden = true, slotCount = 5)
        val brewedSoup = createSoup(id = 10L, recipe = legendaryRecipe)
        val ingredient = createIngredient(id = 1L, name = "양배추", type = IngredientType.COMMON)

        `when`(userRepository.findById(1L)).thenReturn(Optional.of(createUser()))
        `when`(userUnlockedRecipeRepository.findRecipeIdsByUserId(1L)).thenReturn(emptyList())
        `when`(soupRepository.findDistinctRecipeIdsByUserId(1L)).thenReturn(listOf(requireNotNull(brewedSoup.recipe.id)))
        `when`(recipeRepository.findAllByOrderByIdAsc()).thenReturn(listOf(legendaryRecipe))
        `when`(recipeRepository.findById(5L)).thenReturn(Optional.of(legendaryRecipe))
        `when`(recipeIngredientRepository.findAllByRecipeIdOrderBySlotOrderAsc(5L)).thenReturn(
            listOf(createRecipeIngredient(id = 1L, recipe = legendaryRecipe, ingredient = ingredient, slotOrder = 1)),
        )

        val listResponse = recipeQueryService.getRecipes(1L)
        val detailResponse = recipeQueryService.getRecipe(1L, 5L)

        assertEquals("별의 축복 스프", listResponse.legendaryRecipes[0].name)
        assertTrue(listResponse.legendaryRecipes[0].recipeVisible)
        assertEquals("별의 축복 스프", detailResponse.name)
        assertEquals(1, detailResponse.ingredients.size)
    }
}
