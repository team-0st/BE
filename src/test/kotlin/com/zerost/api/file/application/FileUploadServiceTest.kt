...
class FileUploadServiceTest {
    ...
    fun `미션 디렉터리로 파일을 업로드할 수 있다`() {
        ...
        val response = fileUploadService.upload(file, "missions", "device-1")

        assertTrue(response.fileKey.startsWith("missions/device-1/"))
    ...
    fun `빈 파일이면 업로드할 수 없다`() {
        ...
        val exception = assertThrows<BusinessException> {
            fileUploadService.upload(file, "missions", "device-1")
        }
